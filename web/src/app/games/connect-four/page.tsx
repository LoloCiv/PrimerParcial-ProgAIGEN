...
import type { ConnectFourGameState, ConnectFourMove, Difficulty } from '@turn-based-mcp/shared'
...
const GAME_TITLE = 'Connect Four'
const GAME_DESCRIPTION = 'Drop discs and line up four in a row to win! Red always moves first.'

function ConnectFourIcon({ className }: { className: string }) {
  return (
...
export function ConnectFourPage({ initialGameId }: ConnectFourPageProps) {
  const [gameSession, setGameSession] = useState<GameSession<ConnectFourGameState> | null>(null)
  const [isLoading, setIsLoading] = useState(false)
  const [error, setError] = useState<string | null>(null)
  const [gameId, setGameId] = useState<string>('')
  const [availableGames, setAvailableGames] = useState<GameSession<ConnectFourGameState>[]>([])
  const [showCreateForm, setShowCreateForm] = useState(false)
  const [showJoinForm, setShowJoinForm] = useState(false)
  const [aiDifficulty, setAiDifficulty] = useState<Difficulty>('medium')
  const [playerColor, setPlayerColor] = useState<DiscColor>('red')
  ...
  useEffect(() => {
    ...
    if (gameSession.gameState.currentPlayerId === 'ai') {
      const pollInterval = setInterval(async () => {
        try {
          const response = await fetch('/api/games/connect-four')
          if (response.ok) {
            const games = await response.json()
            const updatedGame = games.find((g: GameSession<ConnectFourGameState>) =>
  ...
  const loadAvailableGames = async () => {
    try {
      ...
      if (response.ok) {
        const games = await response.json()
        setAvailableGames(games.filter((game: GameSession<ConnectFourGameState>) =>
  ...
  const startNewGame = async (customGameId?: string) => {
    ...
    try {
      const body: { playerName: string; gameId?: string; difficulty: string; playerColor: DiscColor } = {
        playerName: 'Player',
  ...
  const joinExistingGame = async (existingGameId: string) => {
    ...
    try {
      const games = await fetch('/api/games/connect-four').then(r => r.json())
      const existingGame = games.find((g: GameSession<ConnectFourGameState>) =>
  ...
  const makeMove = async (move: ConnectFourMove) => {
    ...
    try {
      const response = await fetch(`/api/games/connect-four/${gameSession.gameState.id}/move`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const deleteGame = async (gameIdToDelete: string) => {
    setIsDeleting(true)
    try {
      const response = await fetch(`/api/games/connect-four?gameId=${encodeURIComponent(gameIdToDelete)}`, {
        method: 'DELETE',
      })
      ...
      if (gameSession && gameSession.gameState.id === gameIdToDelete) {
        setGameSession(null)
      }
  ...
  const handleDeleteGame = (gameIdToDelete: string) => {
    setGameToDelete(gameIdToDelete)
  ...
  const deleteModal = (
    <ConfirmationModal
      ...
      title="Delete Game"
      message="Are you sure you want to delete this connect four game? This action cannot be undone."
      confirmText="Delete"
      cancelText="Cancel"
    ...
    const sidebar = (
      <>
        ...
        <MCPAssistantPanel
          ...
          gameInstructions={{
            steps: [
              'Ask your AI assistant to play the AI turn',
              "It calls play_game with gameType: 'connect-four'",
              'The AI disc drops into the board automatically'
            ]
          }}
        />
        <GameControls
      ...
      <div className="max-w-2xl mx-auto space-y-8">
        <div className="text-center">
          <div className="w-16 h-16 bg-gradient-to-br from-blue-500 to-indigo-600 rounded-2xl flex items-center justify-center mx-auto mb-4">
            <ConnectFourIcon className="w-8 h-8 text-white" />
          </div>
          ...
          <div className="bg-white/40 dark:bg-slate-800/40 backdrop-blur-sm rounded-2xl shadow-lg border border-white/20 dark:border-slate-700/50 p-6 space-y-6">
            ...
            <div className="space-y-4">
              <div>
                ...
                >
                  <option value="easy">🟢 Easy - Random columns</option>
                  <option value="medium">🟡 Medium - Wins, blocks and plays the center</option>
                  <option value="hard">🔴 Hard - Looks 6 moves ahead</option>
                </select>
              </div>
              <div>
                <label className="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-2">
                ...
                <select
                  value={playerColor}
                  onChange={(e) => setPlayerColor(e.target.value as DiscColor)}
                  className="w-full px-4 py-3 bg-white/60 dark:bg-slate-700/60 border border-slate-200 dark:border-slate-600 rounded-xl focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent transition-all duration-200"
                >
                  <option value="red">🔴 Red - You go first</option>
                  <option value="yellow">🟡 Yellow - AI goes first</option>
                </select>
              </div>
              <div>
                <label className="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-2">
